...
def setup_SimCLR_dataloaders(cfg: DictConfig,
    ...
    wm_train = WM811KDataset(df_train, transform=train_transform, num_views=2)
...
def setup_CNN_dataloaders(cfg: DictConfig,
                          train_transform:nn.Module,
                          test_transform:nn.Module) -> Dict[str, DataLoader]:
    df_train = pd.read_pickle(get_hydra_path(cfg.train.data))
    df_test = pd.read_pickle(get_hydra_path(cfg.test.data))
    wm_train = WM811KDataset(df_train, transform=train_transform)
    wm_train_eval = WM811KDataset(df_train, transform=test_transform)
    wm_test = WM811KDataset(df_test, transform=test_transform)
    ...
    # Training Sampler
    sampling_kwargs = get_sampler(cfg.train.weight_sampler, wm_train.label)

    train_dataloader = DataLoader(wm_train, batch_size=cfg.train.batch_size, **sampling_kwargs,
                                  **dataloader_kwargs)
    train_eval_dataloader = DataLoader(wm_train_eval, shuffle=False, batch_size=cfg.test.batch_size,
                                  **dataloader_kwargs)
    test_dataloader = DataLoader(wm_test, shuffle=False, batch_size=cfg.test.batch_size,
                                 **dataloader_kwargs)
    return {'Train': train_dataloader,
            'Train Eval': train_eval_dataloader,
            'Test': test_dataloader}

def setup_SCWM_dataloaders(cfg: DictConfig,
                          train_transform:nn.Module,
                          test_transform:nn.Module,
                          finetune_transform) -> Dict[str, DataLoader]:
    df_train = pd.read_pickle(get_hydra_path(cfg.train.data))
    df_test = pd.read_pickle(get_hydra_path(cfg.test.data))
    wm_train = WM811KDataset(df_train, transform=train_transform, include_no_crop=True)
    wm_train_eval = WM811KDataset(df_train, transform=test_transform)
    wm_test = WM811KDataset(df_test, transform=test_transform)
    wm_finetune = WM811KDataset(df_train, transform=finetune_transform)
    ...
    test_dataloader = DataLoader(wm_test, shuffle=False, batch_size=cfg.test.batch_size,
                                 **dataloader_kwargs)
    finetune_dataloader = DataLoader(wm_finetune, batch_size=cfg.train.batch_size, **sampling_kwargs,
                                  **dataloader_kwargs)
    return {'Train': train_dataloader,
            'Train Eval': train_eval_dataloader,
            'Test': test_dataloader,
...
def setup_SVM_dataframes(cfg: DictConfig) -> Dict[str, pd.DataFrame]:
    df_train = pd.read_pickle(get_hydra_path(cfg.train.data))
    df_test = pd.read_pickle(get_hydra_path(cfg.test.data))
